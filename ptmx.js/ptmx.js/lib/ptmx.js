...
function Terminal(file, args, opt) {
  ...
  // fork
  term = ptmx.fork(file, args, env, cwd, cols, rows);
...
Terminal.hello_world = hello_world;
Terminal.launch_process = launch_process;
Terminal.create_ptmx = create_ptmx;
Terminal.launch_gdb_process = launch_gdb_process;
/**
 * openpty
 */

Terminal.open = function(opt) {
  var self = Object.ate(Terminal.prototype)
    , opt = opt || {};
...
Terminal.prototype.__defineGetter__('process', function() {
  return ptmx.process(this.fd, this.ptmx) || this.file;
});
...
function create_ptmx() {

  return ptmx.create_ptmx(); 
}


function launch_gdb_process(params) {
  
    program = params["program"];
    tty     = params["tty"];
    
    command_string = s = "gdb -q -nw  -tty "+ tty +" "+program;
    
    command = "gdb";

    args = ["-q", "-nw", "-tty", tty,"--batch", "-x", "runner", program];
    //args = ["-q", "-nw", "-i", "mi", "-tty", tty, program];
    console.log("Launching "+command_string);
    console.log("Args: "+args);
    return spawn(command,args);
}

function launch_process(prog) {
